...
def checks() -> None:
    config = replace(TrainConfig(), learning_starts=3, batch_size=2, max_updates=2,
                     target_sync_every=2, capacity=8, episode_limit=2)
    for expected_decisions, expected_updates in ((4, 2), (3, 1)):
        torch.manual_seed(7)
        ...
        with patch.object(agent, 'sync_target', wraps=agent.sync_target) as sync:
            result = train_loop(task, agent, replay, replace(config, max_updates=expected_updates), records.append)
            assert sync.call_count == (2 if expected_updates == 2 else 1)
