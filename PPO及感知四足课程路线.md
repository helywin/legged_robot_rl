...
**当前进入 201：一串动作怎样分配最后的反馈。** 先沿用已经接触过的观察、奖励、网络和参数更新，在一个小环境里建立策略学习的直觉。随后逐步拼出 PPO，最后能够解释、修改和验证目标仓库的真实训练链路。
...
先建立概念的含义、目的、职责和整体关系，再按需要进入机制与实作。手算、公式和代码是解决具体理解问题的工具，不是每段必经流程。讲参数更新时再解释计算图、梯度和优化器的接力；已有理解不反复机械推演。课件按概念主线重写整合，不按聊天顺序追加。
...
| --- | --- | --- |
| **200 从 DQN 认识 PPO** | PPO 是什么；与 DQN 共享什么；收益估计和动作选择概率有什么区别 | 重写为概念导论：先建立整体关系，再逐步解释机制；不强制手算或练习。原脚手架暂缓，旧版讲解不视为已掌握；按理解进度再安排后续内容 |
| **201 一串动作怎样分配最后的反馈** | 延迟到来的奖励怎样回到之前的动作；一次奖励和整段回报有什么区别 | 当前以 `corridor_ppo.py` 承接完整多步 PPO 项目，先实现连续采样，再分段完成 GAE 与更新；后续 202—205 深入同一模块，不重复搭建。无学习者实现与训练证据时保持 learning |
