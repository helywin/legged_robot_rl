...
经验回放抽样
  ...
  → 奖励与未来价值组成目标 Q 值
  → 预测和目标计算损失
  → 清空旧梯度，backward() 写入在线参数 .grad
  → optimizer.step() 按梯度更新在线网络
  → 按固定间隔同步目标网络
```

## 顺着一条经验检查因果

先确认它是哪次交互，再依次追踪：网络输入是否为动作前观察，索引是否为当时实际动作，target 是否使用同次转移的奖励与下一观察，结束标记是否正确。最后才检查梯度和优化器。网络能运行不能替代这些对应关系。

| 操作 | 会直接改变什么 | 不因此自动改变什么 |
| --- | --- | --- |
| 环境 step 与保存经验 | 场景、记录 | 网络权重 |
| 普通 Q 网络前向与计算 loss | 产生本轮计算结果和求导图 | 网络权重 |
| backward | 累积在线参数梯度 | 参数数值、环境位置 |
| optimizer.step | 按规则更新在线参数及优化器状态 | 目标网络副本、历史奖励 |
| 硬同步目标网络 | 复制在线参数到目标副本 | 不增加一次环境转移 |

上表的普通前向不含会更新统计的特殊层；相关模式边界见 [[概念/PyTorch模块与参数]]。损失函数也未必总是平方损失，参见 [[概念/平方损失与Huber损失]]。
...
> [!info] 概念与学习记录的区别
> 第 033—063 课逐段建立了上述机制；后续完整训练证据见 [[概念/DQN小型基准]]。本页解释数据怎样流动，不据此补记学习者已完成训练。当前课程安排以 [[学习主页]] 为准。
