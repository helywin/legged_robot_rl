...
输入为 [2, -1]，两个动作的权重行分别为 [0.5, 1]、[-1, 0.5]，偏置为 [0.1, 0.2]：

```text
动作 0：0.5 × 2 + 1 × (-1) + 0.1 = 0.1
动作 1：(-1) × 2 + 0.5 × (-1) + 0.2 = -2.3
输出：[0.1, -2.3]
```

第一行权重只负责第一个输出，第二行负责第二个输出；各行都读取同一组输入。权重和偏置来自模型参数，输入来自观察。只做上述乘加不会修改这些参数。

带偏置的层严格说是仿射变换，PyTorch API 仍命名为 `Linear`。这是术语惯例；不影响逐项乘加的理解。
